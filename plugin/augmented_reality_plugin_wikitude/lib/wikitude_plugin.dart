...
class WikitudePlugin {
  static const MethodChannel _channel = const MethodChannel('wikitude_plugin');

  static Future<WikitudeResponse> isDeviceSupporting(
      List<String> features) async {
    final String response =
        await _channel.invokeMethod('isDeviceSupporting', features);
    Map<String, dynamic> responseMap = jsonDecode(response);
    return new WikitudeResponse(
        success: responseMap["success"], message: responseMap["message"]);
  }

  static Future<WikitudeResponse> requestARPermissions(
      List<String> features) async {
    final String response =
        await _channel.invokeMethod('requestARPermissions', features);
    Map<String, dynamic> responseMap = jsonDecode(response);
    return new WikitudeResponse(
        success: responseMap["success"], message: responseMap["message"]);
  }
  ...
  static Future<WikitudeSDKBuildInformation> getSDKBuildInformation() async {
    final String response =
        await _channel.invokeMethod('getSDKBuildInformation');
    Map<String, dynamic> responseMap = jsonDecode(response);
